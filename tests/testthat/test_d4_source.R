...
test_that(
    "get_chroms() returns the full sequence dictionary for files with one or
    multiple tracks",
    {
        exp_seq_dict <-
            list(
...
test_that(
    ...
    {
        query <-
            query(source_example1, "chr1", 43349436, 43349440,
                minus_strand = TRUE)
...
test_that("query() works for a region with multiple non-empty tracks", {
    expect_equal(
        query(source_multitrack, "chr1", 43349436, 43349440,
...
test_that(
    "update_query_results() works for a GRanges with several test intervals",
    {
        # Test cases included:
        ...
        granges_updated <-
            update_query_results(source_example1, granges)
...
test_that(
    "update_query_results() works for a region with multiple non-empty tracks",
    {
        granges <-
            GRanges(
                seqnames = "chr1",
                ranges = IRanges(start = 43349437, end = 43349440),
                strand = "+"
            )
        granges_updated <-
            update_query_results(source_multitrack, granges,
                track = "track1_rep2")
        metadata <-
...
test_that(
    ...
    {
        expect_equal(
            mean(source_example2, "chr3", 0, 5e+07),
            mean(query(source_example2, "chr3", 0, 5e+07)$results)
        )
    }
)
...
test_that("mean() works for a region with multiple non-empty tracks", {
    expect_equal(
        mean(source_multitrack, "chr3", 37011715, 37011720, track = "track2"),
        20.4
    )
})
...
test_that(
    "update_mean() works for a region with multiple non-empty tracks",
    {
        granges <- GRanges(
            seqnames = "chr3",
            ranges = IRanges(start = c(37011716), end = c(37011720))
        )
        granges_updated <-
            update_mean(source_multitrack, granges, track = "track2")
        expect_true(
            identical(mcols(granges_updated)$mean_track2, list(20.4))
        )
    }
)
...
test_that(
    ...
    of a query result",
    {
        expect_equal(
            median(
                source_example2, chr = "chr3", start = 0, end = 5e+07),
            median(
                query(source_example2, chr = "chr3", start = 0,
                    end = 5e+07)$results)
        )
    }
)
...
test_that(
    ...
    {
        expect_equal(
            median(source_example2, chr = "chr3", start = 37011625,
                end = 37011640),
            2
        )
    }
...
test_that("median() works for a region with multiple non-empty tracks", {
    expect_equal(
        median(source_multitrack, chr = "chr3", start = 37011625,
...
test_that(
    "update_median() works for a region with multiple non-empty tracks",
    {
        granges <-
            GRanges(
                seqnames = "chr3",
                ranges = IRanges(start = c(37011626), end = c(37011640))
            )
        granges_updated <-
            update_median(source_multitrack, granges, track = "track2")
        expect_true(
            identical(mcols(granges_updated)$median_track2, list(2))
        )
    }
)
...
test_that(
    ...
    {
        expect_equal(
            percentile(source_multitrack, "chr3", 37011630, 37011646, 1,
                track = "track2"),
            0
        )
        expect_equal(
            percentile(source_multitrack, "chr3", 37011630, 37011646, 50,
                track = "track2"),
            2
        )
        expect_equal(
            percentile(source_multitrack, "chr3", 37011630, 37011646, 90,
                track = "track2"),
            3
        )
        expect_equal(
            percentile(source_multitrack, "chr3", 37011630, 37011646, 100,
                track = "track2"),
            3
        )
    }
...
test_that("percentile() returns 0 for percentile <= 0", {
    expect_equal(
        percentile(source_multitrack, "chr3", 37011640, 37011646, 0,
            track = "track2"),
        0
    )
    expect_equal(
        percentile(source_multitrack, "chr3", 37011640, 37011646, -1,
            track = "track2"),
        0
    )
})
...
test_that("percentile() returns the max for percentile >= 100", {
    ...
    expect_equal(
        percentile(source_multitrack, "chr3", 37011630, 37011646, 101,
            track = "track2"),
        3
    )
})
...
test_that(
    "update_percentile() works for a Granges with several test intervals",
    {
        granges <-
            GRanges(seqnames = "chr3", ranges = IRanges(
                start = c(1e+06, 37011626),
        ...
        granges_updated <-
            update_percentile(source_example2, granges, 50)
        expect_true(
            identical(mcols(granges_updated)$percentile_50, list(0, 2))
        )
    }
)

test_that(
    "update_percentile() works for a region with multiple non-empty tracks",
    {
        granges <- GRanges(
            seqnames = "chr3",
            ranges = IRanges(start = c(37011626), end = c(37011640))
        ...
        granges_updated <-
            update_percentile(source_multitrack,
                granges,
                percentile = 50,
                track = "track2")
        expect_true(
...
test_that("resample() works with default bin size (argument omitted)", {
    expect_equal(
        resample(source_example1, "chr1", 17027540, 17027570, "median")$results,
        c(1)
    )
})
