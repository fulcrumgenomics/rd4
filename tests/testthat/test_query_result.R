...
test_that("source is the correct data path", {
    expect_equal(result_example2$source,
        testthat::test_path("testdata", "example2.d4"))
})

test_that("track works when no track was provided", {
    expect_equal(result_example2$track, "")
    expect_equal(
        query(source_multitrack, "chr3", 37011630, 37011640, track = NA)$track,
        ""
    )
})
...
test_that("track works when there are multiple tracks", {
    expect_equal(
        query(source_multitrack, "chr3", 37011630, 37011640,
            track = "track2")$track,
        "track2"
    )
})
