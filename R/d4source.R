...
query_result <- function(query_result_env, minus_strand=FALSE) {
  ...
  list(
    results = if(minus_strand) rev(results) else results,
    query = list(
...
#' @param minus_strand If TRUE, vector of position scores will read in the minus strand direction from 
#' `end - 1` to `start`.
...
#' 
#' @export
query.D4Source <- function(d4source, chr, start, end, ..., minus_strand = FALSE, track = NA) {
  wrapr::stop_if_dot_args(substitute(list(...)), "query()")
  query_result(d4source$query(chr, start, end, track), minus_strand = minus_strand)
}

#' @export
update_query_results <- function(obj, ...) {UseMethod("update_query_results")}

#' For a set of genomic ranges in a `GRanges` object, extracts each region from the D4 file and adds the query result
#' as a metadata attribute for each genomic range, returning a new `GRanges` object with updated metadata.
#' 
#' @param d4source `D4Source` object
#' @param granges `GRanges` object containing genomic ranges of interest
#' @param track Track name. If omitted or NA, queries the first track.
#' @return New version of `granges` with query results added as the `"query_results"` metadata attribute for each 
#' genomic range. For ranges on the minus strand, the vector of position scores reads in the minus strand direction. If
#' a track is specified, the track name will be included in the metadata column name.
#' @examples
#' d4source <- D4Source(system.file("extdata", "example.d4", package="rd4"))
...
#' 
#' @export
update_query_results.D4Source <- function(d4source, granges, ..., track = NA) {
  wrapr::stop_if_dot_args(substitute(list(...)), "update_query_results()")
  fn <- function(chr, start, end, minus_strand, track) {
    query.D4Source(d4source, chr=chr, start=start, end=end, minus_strand=minus_strand, track=track)
  }
  mcol_name <- if(is.na(track)) "query_results" else paste("query_results", track, sep="_")
  mcols(granges)[[mcol_name]] <- mapply(
    fn,
...
#' @export
update_mean <- function(obj, ...) {UseMethod("update_mean")}

#' For a set of genomic ranges in a `GRanges` object, adds the mean value over the region as a metadata attribute for 
#' each genomic range, returning a new `GRanges` object with updated metadata.
#' 
#' @param d4source `D4Source` object
#' @param granges `GRanges` object containing genomic ranges of interest
#' @param track Track name. If omitted or NA, queries the first track.
#' @return New version of `granges` with region mean added as the `"mean"` metadata attribute for each genomic range. If
#' a track is specified, the track name will be included in the metadata column name.
#' @examples
#' d4source <- D4Source(system.file("extdata", "example.d4", package="rd4"))
#' granges <- GRanges(seqnames=c("chr1"), ranges=IRanges(1000:2000), strand=c("+"))
...
update_mean.D4Source <- function(d4source, granges, ..., track = NA) {
  ...
  fn <- function(chr, start, end, track) {
    mean.D4Source(d4source, chr=chr, start=start, end=end,track=track)
  }
  mcol_name <- if(is.na(track)) "mean" else paste("mean", track, sep="_")
  mcols(granges)[[mcol_name]] <- mapply(
    fn,
    chr = as.vector(seqnames(granges)),
    start = as.vector(start(ranges(granges))) - 1,
...
#' @export
update_median <- function(obj, ...) {UseMethod("update_median")}

#' For a set of genomic ranges in a `GRanges` object, adds the median value over the region as a metadata attribute for 
#' each genomic range, returning a new `GRanges` object with updated metadata.
#' 
#' @param d4source `D4Source` object
#' @param granges `GRanges` object containing genomic ranges of interest
#' @param track Track name. If omitted or NA, queries the first track.
#' @return New version of `granges` with region median added as the `"median"` metadata attribute for each genomic range. 
#' If a track is specified, the track name will be included in the metadata column name.
#' @examples
#' d4source <- D4Source(system.file("extdata", "example.d4", package="rd4"))
#' granges <- GRanges(seqnames=c("chr1"), ranges=IRanges(1000:2000), strand=c("+"))
...
#' 
#' @export
update_median.D4Source <- function(d4source, granges, ..., track = NA) {
  wrapr::stop_if_dot_args(substitute(list(...)), "update_median()")
  fn <- function(chr, start, end, track) {
    median.D4Source(d4source, chr=chr, start=start, end=end,track=track)
  }
  mcol_name <- if(is.na(track)) "median" else paste("median", track, sep="_")
  mcols(granges)[[mcol_name]] <- mapply(
    fn,
    chr = as.vector(seqnames(granges)),
    start = as.vector(start(ranges(granges))) - 1,
...
percentile.D4Source <- function(d4source, chr, start, end, percentile, ..., track = NA) {
  wrapr::stop_if_dot_args(substitute(list(...)), "percentile()")
  d4source$percentile(chr, start, end, track, percentile)
}

#' @export
update_percentile <- function(obj, ...) {UseMethod("update_percentile")}

#' For a set of genomic ranges in a `GRanges` object, adds the percentile value over the region as a metadata attribute 
#' for each genomic range, returning a new `GRanges` object with updated metadata.
#' 
#' @param d4source `D4Source` object
#' @param granges `GRanges` object containing genomic ranges of interest
#' @param percentile Percentile between 0 and 100. Example: percentile 0 is the minimum data value, 50 is the median and 
#' 100 is the maximum.
#' @param track Track name. If omitted or NA, queries the first track.
#' @return New version of `granges` with region percentile added as the `"percentile_<percentile>"` metadata attribute 
#' for each genomic range. The percentile requested (between 0 and 100) is included in the metadata column name. If a 
#' track is specified, the track name will also be included in the column name.
#' @examples
#' d4source <- D4Source(system.file("extdata", "example.d4", package="rd4"))
#' granges <- GRanges(seqnames=c("chr1"), ranges=IRanges(1000:2000), strand=c("+"))
...
#' 
#' @export
update_percentile.D4Source <- function(d4source, granges, percentile, ..., track = NA) {
  wrapr::stop_if_dot_args(substitute(list(...)), "update_percentile()")
  fn <- function(chr, start, end, track) {
    percentile.D4Source(d4source, chr=chr, start=start, end=end, percentile=percentile, track=track)
  }
  mcol_name <- {
    if(is.na(track)) paste("percentile", percentile, sep="_") 
    else paste("percentile", percentile, track, sep="_")
  }
  mcols(granges)[[mcol_name]] <- mapply(
    fn,
    chr = as.vector(seqnames(granges)),
...
resample.D4Source <- function(
  ...
  wrapr::stop_if_dot_args(substitute(list(...)), "resample()")
