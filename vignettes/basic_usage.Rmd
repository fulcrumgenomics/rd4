...
### Exporting data profile from a D4 file to a `GRanges` object

The `to_granges()` function converts data in a `D4Source` object to a `GRanges` object. The data values are stored in a metadata column.
...
to_granges(d4source, chr = "chr1", start = 17027500, end = 17027600)
```
